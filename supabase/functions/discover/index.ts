...
// Deployed to Supabase project REX as the `discover` function (verify_jwt off;
// it's a public movie-data proxy with no sensitive data, gated below).
// Secret required: TMDB_TOKEN (TMDB v4 read access token).
...
// Best-effort per-IP rate limit (per warm instance; not bulletproof, but caps a
// single abuser hammering the key/bill).
...
function discoverParams(type: "movie" | "tv", o: { region: string; withIds?: string; withoutIds?: string; year?: string; actorId?: number; providerId?: number; sort: string; page: string }) {
  ...
  const p: Record<string, string | undefined> = {
    include_adult: "false", watch_region: o.region, page: o.page,
    with_genres: o.withIds, without_genres: o.withoutIds,
  };
  ...
  switch (o.sort) {
    ...
    default: p.sort_by = "popularity.desc"; p["vote_count.gte"] = "150";
  }
  return p;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const type of mediaTypes) {
      ...
      const params = discoverParams(type, { region, withIds, withoutIds, year, actorId, providerId, sort, page });
    ...
    if (sort === "popular" && mediaTypes.length > 1) {
