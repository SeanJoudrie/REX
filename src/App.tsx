...
const sameTitle = (a: Title, b: Title) => a.id === b.id && a.mediaType === b.mediaType

export default function App() {
  const initial = useMemo(loadState, [])
  const [watchlist, setWatchlist] = useState<Title[]>(initial.watchlist)
  const [seen, setSeen] = useState<number[]>(initial.seen)
  ...
  // Load the deck source (sample today, TMDB proxy once configured).
  useEffect(() => {
    let alive = true
    fetchDeck({ mediaTypes: ['movie', 'tv'] }).then(d => { if (alive) setPool(d) })
    return () => { alive = false }
  }, [])

  useEffect(() => { saveState({ watchlist, seen }) }, [watchlist, seen])

  // The live deck: pool minus already-seen, narrowed by the media filter.
  const seenSet = useMemo(() => new Set(seen), [seen])
  const deck = useMemo(
    () => pool.filter(t => !seenSet.has(t.id) && (filter === 'all' || t.mediaType === filter)),
    [pool, seenSet, filter],
  )

  const isSaved = (t: Title) => watchlist.some(w => sameTitle(w, t))

  const like = (t: Title) => {
    setSeen(s => (s.includes(t.id) ? s : [...s, t.id]))
    setWatchlist(w => (w.some(x => sameTitle(x, t)) ? w : [...w, t]))
  }
  const pass = (t: Title) => setSeen(s => (s.includes(t.id) ? s : [...s, t.id]))

  const toggleSave = (t: Title) => {
    if (isSaved(t)) setWatchlist(w => w.filter(x => !sameTitle(x, t)))
    else { setWatchlist(w => [...w, t]); setSeen(s => (s.includes(t.id) ? s : [...s, t.id])) }
  }

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', maxWidth: 640, margin: '0 auto' }}>
      {/* Top bar */}
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px 6px' }}>
        <div style={{ fontWeight: 900, fontSize: 22, letterSpacing: '0.14em' }}>
          R<span style={{ color: '#22c55e' }}>E</span>X
        </div>
        {screen === 'deck' && (
          <div style={{ display: 'flex', gap: 6, background: 'rgba(255,255,255,0.06)', padding: 4, borderRadius: 999 }}>
        ...
        {screen === 'deck' ? (
          deck.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px 24px' }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 800 }}>You've seen everything</div>
              <div style={{ marginTop: 6, fontSize: 13.5, opacity: 0.7 }}>
                {filter !== 'all' ? 'Try switching Film/TV up top, or ' : ''}reset to swipe again.
              </div>
              <button onClick={() => setSeen([])}
  ...
  return (
    <button onClick={onClick}
      ...
      <span style={{ fontSize: 19, position: 'relative' }}>
        {glyph}
        {!!badge && <span style={{ position: 'absolute', top: -4, right: -12, fontSize: 10, fontWeight: 800, minWidth: 16, height: 16, lineHeight: '16px', borderRadius: 999, background: '#22c55e', color: '#06210f' }}>{badge}</span>}
      </span>
